...
export function AddToWatchlistButton({
  ...
  const handleDetailedAdd = async () => {
    ...
    try {
      ...
      watchlistItemData.watched_status = watchedStatus;
      watchlistItemData.personal_rating = personalRating ? parseInt(personalRating) : null;
      watchlistItemData.notes = notes.trim() || null;
      
      if (watchedStatus === 'watched') {
        watchlistItemData.watch_date = new Date().toISOString();
      }
  ...
  const getButtonProps = () => {
    if (isInWatchlist) {
      return {
        icon: 'check',
        text: showStatus ? `In Watchlist${watchlistItem?.watched_status === 'watched' ? ' (Watched)' : ''}` : 'In Watchlist',
  ...
  const isDisabled = isLoading || isPending || addToWatchlist.isPending || removeFromWatchlist.isPending;
      ...
      case 'chip':
        return (
          <Chip
            ...
            style={[
              styles.chip,
              isInWatchlist && { backgroundColor: buttonProps.color + '20' }
            ]}
        ...
        return (
          <View style={styles.buttonContainer}>
            <Button
              ...
              style={[
                styles.button,
                isInWatchlist && { borderColor: buttonProps.color }
              ]}
        ...
        >
          <Card>
            <Card.Title 
              ...
              subtitle={tmdbData.title || tmdbData.name}
            />
